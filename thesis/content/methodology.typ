...
The metrics described in @fun_quality_metrics can all be used a loss functions. The problem that all of them have in common is that non are specific to our task of dereverberation. As reverberation has been shown to either degrade or enhance a signal (cf. @fun_sound_quality) most metrics focus on signal corruptions that purely degrade sound quality (e.g. noise or sampling artifacts). @PESQ comes close beeing a perceptual scale- and shift-invariant metric but as it is made for the evaluation of speech signals, effectiveness in diverse audio signals is doubtful. #cite(<rixPerceptualEvaluationSpeech2001>, form: "prose", style: "chicago-author-date") write: "Certain other applications have not yet been fully characterised or may need parts of the model to be changed. These include: music quality [...]". An alternative lies in the @PEAQ:both model (cf. @fun_peaq).

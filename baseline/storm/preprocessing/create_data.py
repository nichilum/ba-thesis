...
parser.add_argument(
    "--noise",
    type=str,
    choices=["none", "chime", "qut", "wham"],
    default="chime",
    help="Noise",
)
parser.add_argument("--sr", type=int, default=16000)
parser.add_argument(
    "--splits", type=str, default="cv,tr,tt", help="Split folders of the dataset"
)
...
parser.add_argument(
    "--bwe-method", type=str, default="polyphase", choices=["decimate", "polyphase"]
)
...
if "enh" in args.task:
    params["noise_dirs"] = {
        "wham": {
            split: f"/data/lemercier/databases/whamr/wham_noise/{split}"
            for split in splits
        },
        "chime": {
            split: f"/data/lemercier/databases/CHiME3/data/audio/16kHz/backgrounds"
            for split in splits
        },
        "qut": {
            split: f"/data/lemercier/databases/dns_chime3/{dic_splits[split]}"
            for split in splits
        },
    }
...
if args.speech == "wsj0":
    ...
    speech_lists = {
        split: glob.glob(f"{ROOT}/WSJ0/wsj0/{dic_split[split]}/**/*.wav")
        for split in splits
    }
    ...
    for split in splits:
        for spk_idx in range(*ranges[split]):
            speech_lists[split] += glob.glob(
                f"{ROOT}/VCTK-Corpus/wav48/{speakers[spk_idx]}/*.wav"
            )
    ...
    for split in splits:
        ...
        for dr_idx in range(*ranges[split]):
            speech_lists[split] += sorted(
                glob.glob(f"{ROOT}/TIMIT/timit/{splt_dr}/dr{dr_idx}/**/*.wav")
            )
            transcription_lists[split] += sorted(
                glob.glob(f"{ROOT}/TIMIT/timit/{splt_dr}/dr{dr_idx}/**/*.txt")
            )
...
nb_corruptions_per_sample = 1

clean_output_dir = join(output_dir, "audio", "clean")
noisy_output_dir = join(output_dir, "audio", "noisy")
os.makedirs(clean_output_dir, exist_ok=True)
os.makedirs(noisy_output_dir, exist_ok=True)
    ...
    for ic in range(nb_corruptions_per_sample):
        ...
        if "derev" in args.task:
            t60 = np.random.uniform(
                params["t60_range"][0], params["t60_range"][1]
            )  # sample t60
            room_dim = np.array(
                [
                    np.random.uniform(
                        params["dim_range"][2 * n], params["dim_range"][2 * n + 1]
                    )
                    for n in range(3)
                ]
            )  # sample Dimensions
            center_mic_position = np.array(
                [
                    np.random.uniform(
                        params["min_distance_to_wall"],
                        room_dim[n] - params["min_distance_to_wall"],
                    )
                    for n in range(3)
                ]
            ...
            distance_source = (
                1
                / np.sqrt(center_mic_position.ndim)
                * np.linalg.norm(center_mic_position - source_position)
            )
            ...
            noise_floor_power = (
                ...
                / speech.shape[0]
                * np.sum(speech**2)
                * np.power(10, -noise_floor_snr / 10)
            )
            noise_floor_signal = np.random.rand(int(0.5 * args.sr)) * np.sqrt(
                noise_floor_power
            )
        ...
        filename = (
            speech_basename[:-4] + f"_{i_sample * args.corruption_per_sample + ic}"
        )
        if "derev" in args.task:

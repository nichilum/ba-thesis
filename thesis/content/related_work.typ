...
For this thesis, StoRM is relevant because it demonstrates that a generative speech-domain approach can achieve strong dereverberation quality under its intended conditions. However, the scope of the original work remains limited to speech signals and a 16 kHz setting, which limits the representable spectrum to 8 kHz by the Nyquist theorem, which is substantially below the upper range of human hearing and therefore does not preserve the full audible bandwidth of music and other broadband audio material @shannonCommunicationPresenceNoise1949 @isoAcousticsReferenceZero2019a @pumphreyUpperLimitFrequency1950. In addition, the model is not designed around strict real-time constraints, making it less suitable as a direct answer to the low-latency objective of this thesis. StoRM therefore serves here not as a solved solution to real-time dereverberation of diverse audio, but as a high-quality speech-domain reference whose transferability to broader signals and latency-constrained applications must be evaluated separately.
...
DeepFilterNet expands on that idea: "Instead of using a complex mask that is applied per TF-bin, [...] a combination of real-valued gains and a deep filter enhancement component [is used]." @schroterDeepFilterNetLowComplexity2022. Given a noisy signal, DeepFilterNet first transforms it into the frequency-domain using the @STFT. Sampling rates up to 48 kHz and window sizes between 5 ms and 30 ms are supported. Using the frequency-domain representation, @ERB features mimicking human perception are computed. Both the complex features of the @STFT and the @ERB features are used as inputs for the model. A mask is trained on the complex @STFT values, and an encoder--decoder network is trained on the @ERB features, which is later used to weight the complex mask. DeepFilterNet takes advantage of the fact that noise and speech signals usually have a smooth spectral envelope. This allows for a computationally cheap encoder--decoder network.

For this thesis, DeepFilterNet is relevant because it shows how low-complexity speech enhancement can be implemented in the frequency-domain. Regarding audio sample rate, the scope of this project even exceeds ours, but latency is still an issue as window sizes smaller than 5 ms are not supported. Furthermore, since DeepFilterNet's architecture exploits the spectral properties of speech, adapting it for use with diverse audio content proves challenging.


== Quality Net<related_quality_net>
#jojo
Most speech or audio assessment tools need a reference signal. This requirement "considerably restricts the practicality of such assessment tools" @fuQualityNetEndtoEndNonintrusive2018. #cite(<fuQualityNetEndtoEndNonintrusive2018>, form: "prose", style: "chicago-author-date") introduce an end-to-end, non-intrusive speech quality evaluation model named quality net, that aims to predict @PESQ scores without a reference signal.

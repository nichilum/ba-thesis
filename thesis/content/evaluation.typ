...
The analysis of applicable loss functions carried out in @analyze_loss_functions bases itself on the assumption that the size and wetness parameters that were used to parametrize the reverb are good indicators of dereverberation performance. It is reasonable to assume that wetness is a good indicator, as it is used as a factor to multiply the wet signal in the Freeverb implementation. Therefore it has no impact on the reverb tail itself, only making it quieter, which is what we expect of our dereverberation model. The size parameter behaves differently. It indirectly adjusts the intensity of each delayed signal of the comb filters. It is unclear how well a reduction in room size corresponds with reducing reverberation.

@plot_metrics_against_size_and_wet shows every metric performs worse against the size parameter. This might not be the metric's fault; consequently, all conclusions based on the size plots alone require further investigation.

== Perceptual Quality Net<eval_percep_quality_net>
#jojo
Evaluation of the perceptual quality network is done both for the initial simple implementation and the CNN14-based one. The evaluation of the simple @CNN (see @eval_percep_qual_net_init) focuses on the comparison of the two approaches, examining the improvement the CNN14-based implementation made. @eval_percep_qual_net_cnn14 discusses the general applicability of the perceptual quality network as a loss function and its predictive performance compared to other metrics like the @SI-SNR.
...
The output of all prediction heads is compared to their ground truth counterpart using @MSE, @MAE and correlation as comparative metrics. @eval_init_vs_cnn14_compare shows absolute differences between ground truth and prediction as well as relative improvements between the simple @CNN and the CNN14-based implementation for the quality score.
...
Inference speed at 7.74 seconds for 18.2 hours of audio data sampled at 44.1 kHz means that using this network as a loss function should not pose any significant drawbacks in regard to training speed.

None of the shortcomings as identified in @eval_percep_qual_net_cnn14 were addressed or mitigated in any meaningful capacity.
...
#leo
Both Conv-TasNet and StoRM were trained exclusively on speech recordings and had no exposure to music, environmental noise, or other non-speech content. To assess how each model generalizes outside this group, both were applied to 2048 randomly sampled AudioSet clips spanning a wide range of acoustic scenes and event categories. @conv_tasnet_storm_comparison summarizes these metrics, while the full distributions are shown in @boxplot_comparison.
...
#diagram(
  ...
  short-caption: [Loss curves of the generative dereverberation network implementation],
  image(
...
The dereverberation network trained with the @SI-SNR loss was evaluated on a selected slice of the testing set using both spectrograms (@derev_tcn_v4_sisnr and @derev_tcn_v4_sisnr_updated) and objective metrics (@tab_derev_sisnr_results), allowing for comparison with StoRM and Conv-TasNet.
...
@tab_derev_sisnr_results shows a substantial @SI-SNR improvement of $+8.80$ dB on average, which is slightly worse than the validation loss of $16.61$ dB, but still confirms that the dereverberation network is able to generalize to unseen data. @PESQ improves from $1.88$ to $2.94$, an increase of $+1.06$, indicating a meaningful gain in predicted speech quality. Notably, the standard deviation of both @SI-SNR and @PESQ is reduced in the enhanced signal relative to the baseline, suggesting that the model produces more consistent outputs across varying degrees of input reverberation.

The @PEAQ metrics tell a different story. The @ODG decreases marginally by $-0.11$, and the @DI shows a negligible change of $+0.01$, with both deltas exhibiting standard deviations larger than the mean shift itself. This is contrary to the improvements observed in @SI-SNR and @PESQ, again raising the question of whether @PEAQ is a suitable metric for evaluating dereverberation performance, as discussed in @analyze_loss_functions. The @MSE increase from $0.0010$ to $0.0216$ is a direct consequence of the @SI-SNR training objective, which optimizes signal-to-noise ratio rather than sample-wise reconstruction fidelity, and is therefore expected.

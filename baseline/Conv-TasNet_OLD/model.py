...
class ConvTasNet(LightningModule):
    ...
    def forward(self, x):
        ...
        w = self.separation(w)
        m = self.mask(w)
        # m: [B, H*num_spks, T] -> split speakers -> [num_spks, B, H, T]
        # IMPORTANT: softmax must be applied on the feature/channel axis (H), not the speaker axis.
        # The old implementation stacked speakers and applied softmax without specifying dim,
        # which normalizes over the speaker axis and can destabilize training.
        m = torch.stack(torch.chunk(m, chunks=self.num_spks, dim=1), dim=0)
        if self.non_linear is F.softmax:

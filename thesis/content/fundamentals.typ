...
*Self-supervised* learning is a form of unsupervised learning where, instead of relying on external labels, the data itself is used to generate supervisory signals. This is often done by augmentation of the input data to formulate this signal. Self-supervised learning has been successful in recent years, especially in labeled data-scarce domains such as audio processing, as shown by #cite(<baevskiWav2vec20Framework2020>, form: "prose", style: "chicago-author-date"). Two prominent paradigms within self-supervised learning are autoassociative and contrastive methods. In autoassociative learning, the network is trained to reconstruct its own input, forcing it to learn compact internal representations. Contrastive learning instead trains the network to pull similar samples together and push dissimilar ones apart in the representation space, without requiring explicit reconstruction @balestrieroCookbookSelfSupervisedLearning2023.
